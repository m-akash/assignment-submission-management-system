...
/// <summary>
/// Resolves the many-to-many-to-many relationship between Teacher, Course and Class
...
/// course in this class." This is the gate for rule B3 (teachers manage only their
...
public sealed class TeacherAssignment : BaseEntity
{
    ...
    public static TeacherAssignment Create(Guid teacherId, Guid courseId, Guid classId)
    {
        if (teacherId == Guid.Empty || courseId == Guid.Empty || classId == Guid.Empty)
        {
            throw new DomainException("Teacher, course and class ids are all required.");
        }
